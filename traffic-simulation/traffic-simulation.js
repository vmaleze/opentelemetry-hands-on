...
const PRODUCT_API = "http://product:8080/api/products";
const CART_API = "http://shopping-cart:8080/api/shopping-carts";
const ORDER_API = "http://order:8080/api/orders";
const STOCK_API = "http://stock:8080/api/stocks";
...
export default function () {
  ...
  for (let i = 0; i < Math.floor(Math.random() * 3) + 1; i++) {
    ...
    let res = http.post(
      `${STOCK_API}?productId=${product.id}`,
      stockToAddPayload,
      {
    ...
    const addToCartResponse = http.put(
      `${CART_API}/${shoppingCartId}/products`,
      JSON.stringify(selectedProducts[i]),
      {
        headers: { "Content-Type": "application/json" },
      }
    );
    ...
    if (
      !check(addToCartResponse, {
        "PUT /api/shopping-carts/{id}/products status is 201": (res) =>
          res.status === 201,
      })
    ) {
